...
> **MVP 우선순위**: 슬라이스 0 → 1 → 2 → 3 → 4까지 끝나면 "비번으로 댓글 제출 + 관리자가 큐 보고 승인" 흐름이 동작 = **사용 가능한 MVP**.
...
**최소 동작 MVP**: 슬라이스 0+1+2+3+4 (5개 슬라이스). "비번 입력 후 댓글 제출 → 관리자가 큐 조회 → 일괄 승인" 흐름.

**자동화 포함 MVP**: + 슬라이스 6 (워크플로 갱신). 승인된 항목이 실제로 AI에 의해 적용됨.

**완전한 Phase 1**: + 슬라이스 5,7,9 (soft delete, mode 토글, 비상 정지).

**Phase 1 종료**: + 슬라이스 8,10 (프롬프트 외부화, 구조 변경 안전장치).
