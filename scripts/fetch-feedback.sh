...
# .env.local에서 환경변수 로드 (로컬 실행 시)
if [[ -f "$PROJECT_DIR/.env.local" ]]; then
  export $(grep -v '^#' "$PROJECT_DIR/.env.local" | xargs)
fi

# NEXT_PUBLIC_SUPABASE_URL도 지원 (이 프로젝트의 .env 컨벤션)
SUPABASE_URL="${SUPABASE_URL:-${NEXT_PUBLIC_SUPABASE_URL:-}}"

if [[ -z "${SUPABASE_URL}" || -z "${SUPABASE_SERVICE_ROLE_KEY:-}" ]]; then
  echo "ERROR: SUPABASE_URL(또는 NEXT_PUBLIC_SUPABASE_URL) / SUPABASE_SERVICE_ROLE_KEY가 설정되지 않았습니다" >&2
  exit 1
fi
