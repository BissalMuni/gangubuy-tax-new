...
const Sidebar: React.FC<SidebarProps> = ({ collapsed, onCollapse }) => {
  ...
  const { activePath } = useActiveContent();

  // activePath가 있으면 우선 사용, 없으면 location.pathname 사용
  ...
  return (
    ...
    >
      ...
      {/* 메뉴 - 경로에 따라 다른 메뉴 표시 */}
      <Menu
        key={location.pathname.startsWith('/local-tax-theme') ? 'theme' : 'basic'}
        mode="inline"
        selectedKeys={getSidebarSelectedKeys(currentPath)}
